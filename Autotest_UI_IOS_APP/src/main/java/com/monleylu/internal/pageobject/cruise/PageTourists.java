...
public class PageTourists extends BasePage implements CommonActionModule {
    ...
    /**
     * 邮轮品类无法在2／2获取到出游人数目，进入选择出游人页面判断应该多少人出游
     * @return
     */
    public boolean selectTourist(){
        return selectTourist(getTotalTouristToSelect());
    }
    ...
    public boolean selectTourist(int touristNumToSelect) {
        ...
        for (int i = 0; i < sumTourists; i++) {
            ...
            try {
            String nameTmp = listTourists.get(i).findElementByXPath("//XCUIElementTypeStaticText").getText();
...
*/
     ...
     * @return
     */
    public int getTotalTouristToSelect(){
        String str =getDriver().findElementByXPath("//XCUIElementTypeStaticText[contains(@name,'已选择')]").getText();
        return  Integer.parseInt(str.substring(5,str.indexOf("位")));
    }
