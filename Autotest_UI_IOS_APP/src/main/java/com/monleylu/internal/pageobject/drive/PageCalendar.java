...
public class PageCalendar extends BasePage implements CommonActionModule {
    ...
    public boolean waitPageLoadReady(int timeToWaitInSec) {
        return ElementUtil.waitVisibilityOnExpectTime(getDriver(), By.name("选择团期与人数"), timeToWaitInSec);
    }

    public boolean waitLeaveCurrentPage(int timeToWaitInSec) {
        return ElementUtil.waitInVisibilityOnExpectTime(getDriver(), By.xpath("//XCUIElementTypeTable"), timeToWaitInSec);
