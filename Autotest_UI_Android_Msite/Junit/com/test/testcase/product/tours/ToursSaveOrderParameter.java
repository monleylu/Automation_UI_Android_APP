...
import com.testcase.product.tours.ToursOrder;
...
public class ToursSaveOrderParameter {
	...
	public static void setUpBeforeClass() throws Exception {
	    	logger.debug("开始setUpBeforeClass ToursSaveOrderParameter");
	...
	public static void tearDownAfterClass() throws Exception {
		...
		if (isSingle) {
		    ...
		    logger.debug("清除tearDownAfterClass ToursSaveOrderParameter 运行实例");
		}
	...
	public ToursSaveOrderParameter(TestUnit testUnit) {
	...
	public static ArrayList<TestUnit[]>  initTestData(){
	    ...
	    logger.debug("开始参数化ToursSaveOrderParameter");
	    //初始化测试数据参数
	    ...
	    //测试数据默认存储文件和路径
	    testProductsFileName = "ToursSaveOrderParameter_TestProducts.txt";
	    ...
	    //注入登录cookie默认存储路径和文件
	    testCookiesFileName = "ToursSaveOrderParameter_Cookies.txt";
	    ...
	    logger.debug("tours cookieFlag：cookieData = " + cookieFlag +":" + cookieData);
		...
		//设置上层传入的配置文件
		fileProducts = new File(System.getProperty("user.dir")+ testProductsPath  + srcData);
	    }
    ...
    //@Test(timeout=60000)
    @Test
    public void testToursSaveOrder() {
	
	assertTrue("通过cookie注入登录", Login.injectLogin(driver, testUnit.getProductBookInformation().getHostUrl(),testCookiesFileName));
	...
	//SaveOrderResult saveOrderResult = PageProductAction.SaveOrder(testUnit);
	ToursOrder toursOrder = new ToursOrder();
	ToursProduct toursProduct = new ToursProduct(driver);
	SaveOrderResult saveOrderResult = toursOrder.SaveOrder(toursProduct, testUnit);
...
/*	String picHtmlSting="";
	for (int i = 0; i < saveOrderResult.getScreenPics().getPicNames().size(); i++) {
	    String tmp = "<a href=\"./screenshot/" + saveOrderResult.getScreenPics().getPicNames().get(i) +"\">" + saveOrderResult.getScreenPics().getPicNames().get(i) +"</a>";
	    picHtmlSting=picHtmlSting.concat(tmp);
	}
	System.err.println("运行截图：" +  picHtmlSting);*/
	//判断测试用例执行成功与否
