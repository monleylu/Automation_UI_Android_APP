...
public class ToursChangeSingleFlightTest {
	...
	logger.debug("开始参数化ToursChangeSingleFlightTest");
	...
	logger.debug("开始setUpBeforeClass ToursChangeSingleFlightTest");
	if(driverFactory == null){
	    synchronized (UserDriverFactory.class) {
		if(driverFactory == null){
		    logger.debug("ToursChangeSingleFlightTest:初始化进程实例");
    ...
    public ToursChangeSingleFlightTest(TestUnit testUnit) {
	...
	if (isSingle) {
	    ...
	    logger.debug("清除tearDownAfterClass ToursChangeSingleFlightTest 运行实例");
	}
    }
    ...
    @Test
    public void testToursChangeSingleFlight() {
	...
	ToursChangeSingleFlight toursChangeSingleFlight = new ToursChangeSingleFlight();
	
	SaveOrderResult saveOrderResult = toursChangeSingleFlight.checkChangeSingleFlight(toursProduct, testUnit);
	//测试用例执行完毕，截图
