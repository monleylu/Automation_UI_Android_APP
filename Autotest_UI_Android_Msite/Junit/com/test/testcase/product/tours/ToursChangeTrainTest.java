...
public class ToursChangeTrainTest {
	...
	logger.debug("开始参数化ToursChangeTrainTest");
	...
	logger.debug("开始setUpBeforeClass ToursChangeTrainTest");
	if(driverFactory == null){
	    synchronized (UserDriverFactory.class) {
		if(driverFactory == null){
		    logger.debug("ToursChangeTrainTest:初始化进程实例");
    ...
    public ToursChangeTrainTest(TestUnit testUnit) {
	...
	if (isSingle) {
	    ...
	    logger.debug("清除tearDownAfterClass ToursChangeTrainTest 运行实例");
	}
    }
    ...
    @Test
    public void testToursChangeTrain() {
	...
	ToursChangeTrain toursChangeTrain = new ToursChangeTrain();
	
	SaveOrderResult saveOrderResult = toursChangeTrain.checkChangeTrain(toursProduct, testUnit);
	//测试用例执行完毕，截图
