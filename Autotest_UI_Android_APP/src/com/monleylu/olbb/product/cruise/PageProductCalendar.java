...
/*		// TODO Auto-generated method stub
	    ...
	    //邮轮品类目前仍然是老的团期日历页面，有个默认团期，就使用默认团期操作下单
/*	     List<AndroidElement> avaiableDateElement= getDriver().findElementsById("com.monleylu.productdetail:id/min_price_tv");
	...
	public boolean selectAdult(int adultNum) {
		...
		int differenceNum=0;
		//-人+ 这个元素
		AndroidElement locateAdultElement =getDriver().findElementByXPath("//android.widget.LinearLayout[@resource-id='com.monleylu.cruiseship:id/lv_price']/android.widget.RelativeLayout[1]/android.widget.RelativeLayout[@resource-id='com.monleylu.cruiseship:id/ccv_number']");
		
		//get current adult number
		currentAdultNum= Integer.parseInt( locateAdultElement.findElement(By.id("com.monleylu.cruiseship:id/tv_number_content")).getAttribute("text") );
		...
		if (0==differenceNum) {
			...
			for (int i = 0; i < Math.abs(differenceNum); i++) {
			    ...
			    locateAdultElement.findElementById("com.monleylu.cruiseship:id/iv_number_add").click();
			}
		}else {
			//need minus
			for (int i = 0; i < Math.abs(differenceNum); i++) {
			    ...
			    locateAdultElement.findElementById("com.monleylu.cruiseship:id/iv_number_sub").click();
			}
		}
...
/*		String locateChildElementString = "com.monleylu.productdetail:id/ccv_child_number";
		...
		//-人+ 这个元素
		AndroidElement locateAdultElement =getDriver().findElementByXPath("//android.widget.LinearLayout[@resource-id='com.monleylu.cruiseship:id/lv_price']/android.widget.RelativeLayout[2]/android.widget.RelativeLayout[@resource-id='com.monleylu.cruiseship:id/ccv_number']");
		
		//get current adult number
		currentChildNum= Integer.parseInt( locateAdultElement.findElement(By.id("com.monleylu.cruiseship:id/tv_number_content")).getAttribute("text") );
	...
	public boolean clickNextStepBtn() {
		...
		return this.clickNextStepBtn(By.id("com.monleylu.cruiseship:id/tv_next"));
	...
	public boolean waitPageLoadReady(int timeToWaitInSec) {
		...
		//日历框架
		boolean tourDateScrollView = ElementUtil.isVisibility(getDriver(), By.id("com.monleylu.cruiseship:id/layout_content"), timeToWaitInSec);
		//下一步按钮
		boolean nextPageBtnElement = ElementUtil.isVisibility(getDriver(), By.id("com.monleylu.cruiseship:id/tv_next"), timeToWaitInSec);
	...
	public boolean waitLeaveCurrentPage(int timeToWaitInSec){
	    ...
	    if (ElementUtil.isInVisibility(getDriver(), By.id("com.monleylu.cruiseship:id/layout_content"), timeToWaitInSec)) {
		return true;
	    }else{
		return false;
	    }
